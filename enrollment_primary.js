...
d3.csv("all_years_pivot_edit.csv").then(function(data) {
    ...
    let selectedData = data.map(function(d) {
        if (d.Region !== "" && d.Country !== "" && d.Income !== "" && d.GovtExpEd !== "" && d.EnrollmentPrimary !== "") {
            return {
                Region: d.Region,
                Country: d.Country,
                Income: d.Income,
                GovtExpEd: +d.GovtExpEd,
                EnrollmentPrimary: +d.EnrollmentPrimary,
            };
        }
    ...
    const y = d3.scaleLinear()
        .domain(d3.extent(selectedData, function(d) { return d.EnrollmentPrimary; }))
        .range([height, 0])
    svg.append("g")
    ...
    const dots = svg.append("g")
        ...
        .on("mouseover", function(d) {
            ...
            tooltip.html("<b>Country</b>: " + d.Country +
                    ...
                    "<br><b>School enrollment, primary (gross), gender parity index (GPI)</b>: " + d.EnrollmentPrimary)
    ...
    dots.transition()
        ...
        .attr("cy", function(d) { return y(d.EnrollmentPrimary); })
    ...
    let differences = selectedData.map(function(d) {
        return Math.abs(d.EnrollmentPrimary - 1);
    });
    ...
    const yPixel = y(usData.EnrollmentPrimary);
