...
d3.csv("all_years_pivot_edit.csv").then(function(data) {
    ...
    let selectedData = data.map(function(d) {
        if (d.Region !== "" && d.Country !== "" && d.Income !== "" && d.GovtExpEd !== "" && d.EnrollmentTertiary !== "") {
            return {
                ...
                EnrollmentTertiary: +d.EnrollmentTertiary,
            };
        }
    ...
    svg.append("text")
        ...
        .attr("x", width / 2)
        .attr("y", -20)
        .text("Tertiary Enrollment (GPI) vs. Government Expenditure on Education")
    ...
    dots.transition()
        ...
        .attr("cy", function(d) { return y(d.EnrollmentTertiary); })
    ...
    let differences = selectedData.map(function(d) {
        return Math.abs(d.EnrollmentTertiary - 1);
    });
    ...
    const yPixel = y(usData.EnrollmentTertiary);
  
    //Annotation
    const annotations = [{
        note: {
        label: `\n#${y_rank} out of ${y_count} in tertiary enrollment (GPI), #${x_rank} out of ${x_count} in Government expenditure on education.`,
        title: "United States",
        wrap: 150,
        align: "left",
        color: ["#5b5b5b"]
